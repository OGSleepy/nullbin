...
export function Editor() {
  ...
  // Generate QR code whenever a link is ready
  useEffect(() => {
    if (status.type === "done") {
      QRCode.toDataURL(status.link, { width: 200, margin: 2, color: { dark: "#ffffff", light: "#111827" } })
        .then(setQrDataUrl)
      ...
      <div className="space-y-6">
        ...
        {qrDataUrl && (
          <div className="flex flex-col items-center gap-2">
            <p className="text-xs text-gray-500 font-mono uppercase tracking-wider">Scan to open</p>
            <img
              src={qrDataUrl}
              alt="QR code for share link"
              className="w-40 h-40 rounded-xl border border-gray-800"
            />
          </div>
        )}
